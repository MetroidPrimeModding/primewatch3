...
impl DolphinMemoryAccess {
  fn get_dolphin_pids(&mut self) -> Vec<Pid> {
  ...
  fn attach_to_process(&mut self, pid: i32) -> bool {
  ...
  fn detach_from_process(&mut self) {
    #[cfg(target_os = "linux")]
    {
  ...
  fn get_attached_pid(&self) -> i32 {
    self.attached_pid
  }

  fn dolphin_memcpy(&self, dest: &mut [u8], offset: usize, size: usize) -> bool {
    #[cfg(target_os = "linux")]
    {
