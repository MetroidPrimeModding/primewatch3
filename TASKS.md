...
- [ ] **TODO (found in P3.1, out of scope):** `GameMemory::new()` returns a struct with an inline
  `[u8; 0x1800000]` (~24 MiB) and `GameMemory::load_from_file` materialises another ~24 MiB array on
  the stack via `Vec::<u8>::try_into::<[u8; N]>`. Both overflow a 2 MiB thread stack (and likely the
  8 MiB main stack once `App` also holds it). Should become a heap constructor
  (`Box::new_zeroed`) + read-into-`self.data`. Not fixed here — P3.1's tests work around it with a
  local `blank()` helper. Likely wants doing in/before P3.2.
