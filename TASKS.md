...
- [ ] **P8.4.3** `renderEntities` dispatch + player/trigger/dock/actor/physicsActor draw fns (geometry only). — `TODO` · ports `WorldRenderer.cpp:312-318,336,531-773`: entity walk, per-class dispatch via `extends_class`, `drawPlayer` + trigger/dock/actor/physicsActor. `update` gains the `objects` map + `highlighted` set; draw fns accumulate into the `WorldRenderer`'s immediate buffers.
